...
void ParticleSystem::flock(const Particle& particle, float radius, float maxforce) {
	float targetX = particle.x;
	float targetY = particle.y;
	float minX = targetX - radius;
	float minY = targetY - radius;
	float maxX = targetX + radius;
	float maxY = targetY + radius;
			...
			for(int i = 0; i < n; i++) {
				...
				if(curParticle.x > minX && curParticle.x < maxX &&
				   curParticle.y > minY && curParticle.y < maxY) {
					...
					length = xd * xd + yd * yd;
					if(length > 0 && length < maxrsq) {
						//cal avg pos of particles within radius and add cohesion force
						float avgx = (curParticle.x + targetX)/2 - curParticle.x;
						float avgy = (curParticle.y + targetY)/2 - curParticle.y;
						//limit max force
						if (avgx > maxforce) avgx = maxforce;
						if (avgx < -maxforce) avgx = -maxforce;
						if (avgy > maxforce) avgy = maxforce;
						if (avgy < -maxforce) avgy = -maxforce;
						curParticle.xf = (curParticle.xf + avgx)/2;
						curParticle.yf = (curParticle.yf + avgy)/2;
						float avgxv = (curParticle.xv + particle.xv)/2;
						float avgyv = (curParticle.yv + particle.yv)/2;
						curParticle.xf += avgxv - curParticle.xv;
						curParticle.yf += avgyv - curParticle.yv;
					}
				}
			}
		}
