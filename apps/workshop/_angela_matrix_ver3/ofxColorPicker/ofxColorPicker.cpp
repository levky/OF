...
ofxColorPicker::ofxColorPicker(string name, float *values) : ofxSimpleGuiControl(name) {
	this->values=values;
	controlType="ColorPicker2d";
	init();
}

void ofxColorPicker::loadFromXML(ofxXmlSettings &XML) {
	for(int i=0; i<4; i++) {
		set(XML.getValue("controls:" + controlType + "_" + key + ":values_" + ofToString(i), 0.0f), i);
	}
}

void ofxColorPicker::saveToXML(ofxXmlSettings &XML) {
...
void ofxColorPicker::set(float f, int i) {
	if(f < 0) f = 0;
	else if(f > 1) f =1;
	values[i] = f;
}
...
void ofxColorPicker :: init()
{
	...
	//setSize(0,0,ofGetHeight()/2, ofGetHeight()/2);
	//add
	ofColor c;
	c.r=values[0]*255;
	c.g=values[1]*255;
	...
	for(int i=0; i<4; i++) {
		pct[i] = ofMap(values[i], 0,1, 0.0, width);
		barwidth[i] = pct[i];
	}
...
float ofxColorPicker::getValue(int i) {
	return values[i];
}
...
void ofxColorPicker :: setPos( float x, float y )
{
	...
	checkDimensions( x+100, y+100, dimensions.width, dimensions.height );
}
...
bool ofxColorPicker :: checkDimensions( int x, int y, int w, int h )
{
	if( dimensions.x != x || dimensions.y != y || dimensions.width != w || dimensions.height != h )
	{
		...
		rectColorBox.y				= (int)( dimensions.y + colorWheelPad * 2 + colorWheelRadius * 2);
		//rectColorBox.y				= (int)( rectColorScaleBar.y + rectColorScaleBar.height + colorWheelPad );
		//change 2 to 3
		rectColorBox.width			= (int)( dimensions.width - colorWheelPad *3 );
		rectColorBox.height			= (int)((dimensions.width - colorWheelPad*2) /2);
...
void ofxColorPicker :: update()
{
	...
	values[0]=color.r/255;
	values[1]=color.g/255;
	values[2]=color.b/255;
}
