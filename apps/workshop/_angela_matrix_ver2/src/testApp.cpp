...
//--------------------------------------------------------------
//addNewCell() was used to add a new cell into the board. It will ramdomly pick a position in the board and add a new cell into it.
//--------------------------------------------------------------
...
//--------------------------------------------------------------
//inputslider is from 1 to 10,
//--------------------------------------------------------------
void testApp::maintainCircle2(float x){
	float targetcells=5.0595*x*x+65.89*x-55.9495;
	//if less than 15 circle alive
	if (NumOfALiveCircle<15){
		pcounter=1;
		deadspeed=0;
	}
	
	//if slider is less than 100
	else if (targetcells<100 ){
		
		//if want to move alive cirle up , no cell is dying, and add new cell when hit only once
		if(targetcells>NumOfALiveCircle){
			pcounter=1;
			deadspeed=0;
		}
		else{
	...
	//
	//if slider is less than 700
	else if (targetcells<700 && targetcells>99 ){
		if(targetcells>NumOfALiveCircle){
			if(NumOfALiveCircle<100) {
				pcounter=1;
				deadspeed=0;
			} else {
				pcounter=6;
	...
	//if slider is less than 800
	else if (targetcells<800 && targetcells>699){
		if(targetcells>NumOfALiveCircle){
			if(NumOfALiveCircle<100) {
				pcounter=1;
				deadspeed=0;
	...
	//if slider is less than 900
	else if (targetcells<900 && targetcells>799){
		if(targetcells>NumOfALiveCircle){
			if(NumOfALiveCircle<100) {
				pcounter=1;
				deadspeed=0;
	...
	//if slider is less than 1000
	else if (targetcells<1000 && targetcells>899){
		if(targetcells>NumOfALiveCircle){
			if(NumOfALiveCircle<100) {
				pcounter=1;
				deadspeed=0;
	...
	//if slider is less than 1200
	else if (targetcells<1200 && targetcells>999){
		//add more cirle into the screen
		if(targetcells>NumOfALiveCircle){
			if(NumOfALiveCircle<100) {
				pcounter=1;
				deadspeed=0;
...
//send the initial message to iphone/ipod/ipad
	...
	//setting of gui
		gui.addSlider("Slider: ", slider,1,10);
	gui.addColorPicker("color: ", myColors);
...
void testApp::draw(){
	...
	float x=slider;
	float pslider=5.0595*x*x+65.89*x-55.9495;
	...
	if(flag){
		...
		temp+="Target circles: "+ofToString(pslider);
